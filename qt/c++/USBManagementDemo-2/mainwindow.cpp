...
int MainWindow::SetupUI()
{
    ...
    for (int i = 0; i < 4; i++) {
        ...
        QListWidget *mAppList = new QListWidget();  // 程序组的展示程序的列表组件
        ...
        // 协议版本
        QPushButton *mProtocolVersionBtn = new QPushButton(QObject::tr("协议V1.0"));
        //mProtocolVersionBtn->setFixedSize(QSize(64, 18));
        ...
        if (i == 3) {
            QLabel *mNoOpenAppLabel = new QLabel(QObject::tr("暂无打开的应用"));
            mNoOpenAppLabel->setAlignment(Qt::AlignHCenter);
            mNoOpenAppLabel->setObjectName("noUsbTipImageLabel");
            mNoOpenAppLabel->setAlignment(Qt::AlignCenter);
            mNoOpenAppLabel->setAlignment(Qt::AlignHCenter);
            mAppGroupLayout->addWidget(mNoOpenAppLabel);
            mAppGroupLayout->setAlignment(Qt::AlignCenter);
            mGroupHandler->setChecked(false); // 默认不选中
            // 设置 QRadioButton 为不可选中状态
            mGroupHandler->setEnabled(false); // 禁用按钮
            mGroupHandler->setToolTip("管理员已禁止此外设接入该位置");
            continue;
        }
