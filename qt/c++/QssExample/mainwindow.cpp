...
QString loadStyleSheet(const QString &fileName)
{
    ...
    if (!qssFile.open(QFile::ReadOnly)) {
        return QString(); // 如果文件无法打开，返回空字符串
    }

    QTextStream qssStream(&qssFile);
    return qssStream.readAll(); // 读取文件内容
}

QString loadStyleSheet2(const QString &fileName)
{
    QFile *qssFile = new QFile(fileName);
    if (!qssFile->open(QFile::ReadOnly)) {
        return QString(); // 如果文件无法打开，返回空字符串
    }
    ...
    , ui(new Ui::MainWindow)
{
    ...
    // 加载 QSS 样式
    QString styleSheet = loadStyleSheet2("://qss/style.qss");
    this->setStyleSheet(styleSheet); // 应用样式
