...
static struct usbredirhost_bulk_cache_list_item **s_bulk_cache_list = NULL;
static int s_bulk_cache_count = 0;
void usbredirhost_free_bulk_cache_list()
{
    if (s_bulk_cache_list) {
        free(s_bulk_cache_list);
        s_bulk_cache_list = NULL;
    }
    s_bulk_cache_count = 0;
}
void usbredirhost_init_bulk_cache_list(int count,
    struct usbredirhost_bulk_cache_list_item **list)
{
    if (list == NULL) {
        return;
    }
    ...
    usbredirhost_free_bulk_cache_list();

    s_bulk_cache_list  = list;
    s_bulk_cache_count = count;
}
...
int main()
{
    ...
    usbredirhost_init_bulk_cache_list(count, bulk_cache_list);
    printf("hejian no_transfers %d\n", s_bulk_cache_list[0]->param[0].no_transfers);
    printf("hejian no_transfers %d\n", s_bulk_cache_list[1]->param[0].no_transfers);
    printf("hejian no_transfers %d\n", s_bulk_cache_list[2]->param[0].no_transfers);
    return 0;
}
