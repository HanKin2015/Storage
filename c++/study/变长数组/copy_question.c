...
int main()
{
    ...
    //memcpy(bulk_cache_packet, (p->id)+ID_LEN, len-ID_LEN);    // 方法一
    //memcpy(bulk_cache_packet, (&p)+ID_LEN, len-ID_LEN);       // 不可取
    memcpy(bulk_cache_packet, ((char *)p)+ID_LEN, len-ID_LEN);  // 方法二（将结构体指针强制转换为指向其首成员的指针类型）
