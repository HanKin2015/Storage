...
## 3、区别
std::lock本质：一个函数模板，用于同时锁定多个互斥量（std::mutex 或其派生类型）。
std::lock_guard本质：一个模板类，用于自动管理互斥量的锁定与解锁（RAII 机制）。
std::unique_lock本质：更灵活的 RAII 互斥量管理类，功能比 lock_guard 更丰富。在自动管理的基础上，支持手动控制锁定 / 解锁时机。
std::shared_lock（C++17 起）本质：用于共享所有权的 RAII 管理类，配合 std::shared_mutex 使用。
