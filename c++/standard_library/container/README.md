...
## 9、并发集合
C++ 标准库的大部分容器（如 std::vector、std::map、std::queue 等）本身不是线程安全的，多线程同时读写可能导致未定义行为。

C++ 标准库中原生线程安全的容器极少，主要有两个：
std::atomic<T>：原子类型容器（针对基本类型），支持原子读写操作，可视为线程安全的 “单元素容器”。
std::shared_ptr：智能指针本身不是容器，但它的引用计数操作是线程安全的（C++11 起），可安全用于多线程共享对象的生命周期管理。